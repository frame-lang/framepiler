...
impl SystemParser {
    pub fn parse_module(bytes: &[u8], lang: TargetLanguage) -> ModuleAst {
        ...
        while i < n {
            ...
            let mut persist_attr: Option<crate::frame_c::compiler::ast::PersistAttrAst> = None;
            ...
            if j + 8 <= n && &bytes[j..j+8] == b"@@system" {
                ...
                // V4: Could be @@persist or another attribute
                j += 2;
                let attr_start = j;
                ...
                if attr_start < j {
                    ...
                    if attr.as_str() == "persist" {
                        persist_attr = Some(crate::frame_c::compiler::ast::PersistAttrAst {
                            save_name: None,
                            restore_name: None,
                        });
                        // Skip whitespace after @@persist
                        while j < n && is_space(bytes[j]) {
                            j += 1;
                        }
                    } else {
                        // Unknown @@ attribute; for now, reset j and let system check handle it
                        j = i;
                        while j < n && is_space(bytes[j]) {
                            j += 1;
                        }
