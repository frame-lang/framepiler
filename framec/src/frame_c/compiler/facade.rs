...
pub fn validate_wrappers(spliced_text: &str, lang: TargetLanguage) -> Vec<NativeDiagnostic> {
    let require_semicolon = matches!(lang,
        TargetLanguage::TypeScript | TargetLanguage::JavaScript
        | TargetLanguage::C | TargetLanguage::Cpp
        | TargetLanguage::Java | TargetLanguage::CSharp
        | TargetLanguage::Rust | TargetLanguage::Php
    );
    let forbid_semicolon = matches!(lang,
        TargetLanguage::Python3 | TargetLanguage::Ruby
    );
    // Languages where wrapper validation applies
    let has_wrappers = matches!(lang,
        TargetLanguage::Python3 | TargetLanguage::TypeScript | TargetLanguage::JavaScript
        | TargetLanguage::Rust | TargetLanguage::C | TargetLanguage::Cpp
        | TargetLanguage::Java | TargetLanguage::CSharp | TargetLanguage::Go
        | TargetLanguage::Php | TargetLanguage::Kotlin | TargetLanguage::Swift
        | TargetLanguage::Ruby | TargetLanguage::Lua
    );
    if !has_wrappers {
        return Vec::new();
    }
    ...
    while i < n {
        ...
        while s < line_end && (bytes[s] == b' ' || bytes[s] == b'\t') { s += 1; }
        ...
        if is_transition {
            if let Some((arg_start, arg_end)) = paren_payload(bytes, s, line_end) {
                if let Some((false, msg)) = check_transition_first_arg(bytes, arg_start, arg_end) {
                    diags.push(NativeDiagnostic { start: s, end: line_end, message: msg });
                }
            }
        }
        ...
        if forbid_semicolon && has_semi {
            diags.push(NativeDiagnostic {
                start: s, end: line_end,
                message: format!("semicolon not allowed in {} wrapper", match lang {
...
fn has_balanced_parens(hay: &[u8], start: usize, end: usize) -> bool {
    ...
    let mut i = start;
    while i < end {
        if hay[i] == b'(' { seen_open = true; depth += 1; }
        if hay[i] == b')' { depth -= 1; }
        i += 1;
    }
    !seen_open || depth == 0
...
fn paren_payload(hay: &[u8], start: usize, end: usize) -> Option<(usize, usize)> {
    ...
    while i < end {
        if hay[i] == b'(' { depth += 1; }
            ...
            depth -= 1;
            if depth == 0 { return Some((i - (i - start - 1) + start, i)); }
        }
        i += 1;
    }
    None
...
fn check_transition_first_arg(hay: &[u8], arg_start: usize, arg_end: usize) -> Option<(bool, String)> {
    let mut i = arg_start;
    while i < arg_end && (hay[i] == b' ' || hay[i] == b'\t') { i += 1; }
    if i >= arg_end || (hay[i] != b'\'' && hay[i] != b'"') {
        return Some((false, "transition wrapper: first argument must be quoted state".into()));
    }
    let q = hay[i];
    i += 1;
    let name_start = i;
    while i < arg_end && (hay[i].is_ascii_alphanumeric() || hay[i] == b'_') { i += 1; }
    if i == name_start {
