...
use super::import_scanner::{ImportScanResult, ImportScanner};
...
impl ModulePartitioner {
    pub fn partition(
        bytes: &[u8],
        lang: TargetLanguage,
    ) -> Result<ModulePartitions, ModulePartitionError> {
        ...
        match lang {
            crate::frame_c::visitors::TargetLanguage::Python3 => {
                ...
                imports = res.spans;
                import_issues.extend(res.issues);
        ...
        let outline_items: Vec<OutlineItem> = OutlineScanner
            .scan(bytes, i, lang)
            .map_err(|e| ModulePartitionError(e.message))?;
        let bodies: Vec<BodyPartition> = outline_items
            .into_iter()
            .map(|it| BodyPartition {
