...
// Dogfooded body closer â Swift language brace matcher.
// Swift has: //, /* */ (nestable!), "...", """...""" multi-line strings, char literals in strings.
// String interpolation \(expr) can contain braces â handled by tracking depth within strings.
//
// State machine flow:
//   $Init.scan() â $Scanning.$>() â $InString/$InRawString/$InLineComment/$InBlockComment

#[allow(dead_code)]
struct SwiftBodyCloserFsmFrameEvent {
    message: String,
    parameters: std::collections::HashMap<String, Box<dyn std::any::Any>>,
}

impl Clone for SwiftBodyCloserFsmFrameEvent {
    fn clone(&self) -> Self {
        Self {
...
struct SwiftBodyCloserFsmFrameContext {
    event: SwiftBodyCloserFsmFrameEvent,
    _return: Option<Box<dyn std::any::Any>>,
    _data: std::collections::HashMap<String, Box<dyn std::any::Any>>,
}

impl SwiftBodyCloserFsmFrameContext {
    fn new(event: SwiftBodyCloserFsmFrameEvent, default_return: Option<Box<dyn std::any::Any>>) -> Self {
        Self {
            event,
...
impl SwiftBodyCloserFsm {
    pub fn new() -> Self {
        ...
        let __frame_event = SwiftBodyCloserFsmFrameEvent::new("$>");
        let __ctx = SwiftBodyCloserFsmFrameContext::new(__frame_event, None);
    ...
    fn __kernel(&mut self) {
        ...
        while self.__next_compartment.is_some() {
            ...
            let exit_event = SwiftBodyCloserFsmFrameEvent::new_with_params("<$", &self.__compartment.exit_args);
            self.__router(&exit_event);
            // Switch to new compartment
            self.__compartment = next_compartment;
            // Enter new state (or forward event)
            if self.__compartment.forward_event.is_none() {
                let enter_event = SwiftBodyCloserFsmFrameEvent::new_with_params("$>", &self.__compartment.enter_args);
    ...
    fn __router(&mut self, __e: &SwiftBodyCloserFsmFrameEvent) {
    ...
    fn __push_transition(&mut self, new_compartment: SwiftBodyCloserFsmCompartment) {
    ...
    pub fn scan(&mut self) {
        let mut __e = SwiftBodyCloserFsmFrameEvent::new("scan");
        let mut __ctx = SwiftBodyCloserFsmFrameContext::new(__e, None);
    ...
    fn _s_Init_scan(&mut self, __e: &SwiftBodyCloserFsmFrameEvent) {
        let mut __compartment = SwiftBodyCloserFsmCompartment::new("Scanning");
        __compartment.parent_compartment = Some(Box::new(self.__compartment.clone()));
        self.__transition(__compartment);
        return;
    }

    fn _s_Scanning_enter(&mut self, __e: &SwiftBodyCloserFsmFrameEvent) {
        ...
        while self.pos < n {
            ...
            if b == b'\n' {
                ...
                self.pos += 2;
                let mut __compartment = SwiftBodyCloserFsmCompartment::new("InLineComment");
                __compartment.parent_compartment = Some(Box::new(self.__compartment.clone()));
                self.__transition(__compartment);
                return;
            } else if b == b'/' && self.pos + 1 < n && self.bytes[self.pos + 1] == b'*' {
                ...
                let mut __compartment = SwiftBodyCloserFsmCompartment::new("InBlockComment");
                __compartment.parent_compartment = Some(Box::new(self.__compartment.clone()));
                self.__transition(__compartment);
                return;
            } else if b == b'"' && self.pos + 2 < n && self.bytes[self.pos + 1] == b'"' && self.bytes[self.pos + 2] == b'"' {
                self.pos += 3;
                let mut __compartment = SwiftBodyCloserFsmCompartment::new("InRawString");
                __compartment.parent_compartment = Some(Box::new(self.__compartment.clone()));
                self.__transition(__compartment);
                return;
            } else if b == b'"' {
                self.pos += 1;
                let mut __compartment = SwiftBodyCloserFsmCompartment::new("InString");
                __compartment.parent_compartment = Some(Box::new(self.__compartment.clone()));
                self.__transition(__compartment);
                return;
            } else if b == b'{' {
    ...
    fn _s_InBlockComment_enter(&mut self, __e: &SwiftBodyCloserFsmFrameEvent) {
        let n = self.bytes.len();
        while self.pos + 1 < n {
    ...
    fn _s_InRawString_enter(&mut self, __e: &SwiftBodyCloserFsmFrameEvent) {
        let n = self.bytes.len();
        while self.pos + 2 < n {
    ...
    fn _s_InString_enter(&mut self, __e: &SwiftBodyCloserFsmFrameEvent) {
        let n = self.bytes.len();
        while self.pos < n {
    ...
    fn _s_InLineComment_enter(&mut self, __e: &SwiftBodyCloserFsmFrameEvent) {
        let n = self.bytes.len();
