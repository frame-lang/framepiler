...
//
// Source: ruby_skipper.frs (Frame specification)
// Generated: ruby_skipper.gen.rs (via framec compile -l rust)
// This file: glue module wiring generated FSM to SyntaxSkipper trait
//
// To regenerate:
//   ./target/release/framec compile -l rust -o /tmp framec/src/frame_c/compiler/native_region_scanner/ruby_skipper.frs
//   cp /tmp/ruby_skipper.rs framec/src/frame_c/compiler/native_region_scanner/ruby_skipper.gen.rs
...
include!("ruby_skipper.gen.rs");

use super::unified::*;
use super::*;
use crate::frame_c::compiler::body_closer::ruby::BodyCloserRuby;
use crate::frame_c::compiler::body_closer::BodyCloser;

pub struct NativeRegionScannerRuby;

/// Ruby syntax skipper - handles # comments and string literals
pub struct RubySkipper;

impl SyntaxSkipper for RubySkipper {
    fn body_closer(&self) -> Box<dyn BodyCloser> {
        Box::new(BodyCloserRuby)
    }

    fn skip_comment(&self, bytes: &[u8], i: usize, end: usize) -> Option<usize> {
        let mut fsm = RubySyntaxSkipperFsm::new();
...
impl NativeRegionScanner for NativeRegionScannerRuby {
    fn scan(&mut self, bytes: &[u8], open_brace_index: usize) -> Result<ScanResult, ScanError> {
        super::unified::scan_native_regions(&RubySkipper, bytes, open_brace_index)
    }
}
