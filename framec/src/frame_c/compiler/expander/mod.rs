    ...
    fn expand(&self, mir: &MirItem, indent: usize, system_ctx: Option<&str>) -> String {
        ...
        match mir {
            MirItem::Transition {
                ...
                let id = match system_ctx {
                    Some(sys) => format!("__{}_state_{}", sys, target),
                    None => target.clone(),
                };
                ...
                if !state_args.is_empty() {
                    ...
                    let rendered: Vec<String> = state_args
                        .iter()
                        .map(|t| {
            ...
            MirItem::TransitionForward { target, .. } => {
                ...
                out.push_str(&format!(
                    "{}return self._frame_router(__e, self._compartment)\n",
                    pad
                ));
                out
            }
    ...
    fn expand(&self, mir: &MirItem, indent: usize, system_ctx: Option<&str>) -> String {
        ...
        match mir {
            ...
            MirItem::TransitionForward { target, .. } => {
                ...
                out.push_str(&format!("{}{{\n", pad));
                out.push_str(&format!(
                    "{}const __frameNextCompartment = new FrameCompartment(\"{}\");\n",
                    pad, id
                ));
                out.push_str(&format!(
                    "{}this._frame_transition(__frameNextCompartment);\n",
                    pad
                ));
                out.push_str(&format!(
                    "{}return this._frame_router(__e, this._compartment);\n",
                    pad
                ));
                out.push_str(&format!("{}}}\n", pad));
                out
    ...
    fn expand(&self, mir: &MirItem, indent: usize, system_ctx: Option<&str>) -> String {
        ...
        match mir {
            MirItem::Transition {
                ...
                let mut out = String::new();
                out.push_str(&format!(
                    "{}FrameCompartment next_compartment = frame_compartment_new({});\n",
                    pad, id
                ));
    ...
    fn expand(&self, mir: &MirItem, indent: usize, system_ctx: Option<&str>) -> String {
        ...
        match mir {
            MirItem::Transition { target, .. } => {
                ...
                out.push_str(&format!(
                    "{}FrameCompartment nextCompartment = new FrameCompartment(\"{}\");\n",
                    pad, id_raw
                ));
...
impl FrameStatementExpander for RustExpander {
    fn expand(&self, mir: &MirItem, indent: usize, system_ctx: Option<&str>) -> String {
        ...
        // on the generated system struct. Otherwise, keep the free-function
    ...
    fn expand(&self, mir: &MirItem, indent: usize, _system_ctx: Option<&str>) -> String {
        ...
        match mir {
            MirItem::Transition {
                ...
                let arglist = if state_args.is_empty() {
                    String::new()
                } else {
...
impl FrameStatementExpander for CppFacadeExpander {
    fn expand(&self, m: &MirItem, i: usize, s: Option<&str>) -> String {
        CFacadeExpander.expand(m, i, s)
    }
}
impl FrameStatementExpander for JavaFacadeExpander {
    fn expand(&self, m: &MirItem, i: usize, s: Option<&str>) -> String {
        CFacadeExpander.expand(m, i, s)
    }
}
