    ...
    fn emit(&self, node: &CodegenNode, ctx: &mut EmitContext) -> String {
        match node {
            ...
            CodegenNode::Method {
                ...
                // For async Rust kernel/router: use boxed future to break recursion
                let use_boxed_future = *is_async && (name == "__kernel" || name == "__router");
                if use_boxed_future {
                    // Use named lifetime 'a to tie &mut self and parameter references together
