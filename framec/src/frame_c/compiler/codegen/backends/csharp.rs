...
impl LanguageBackend for CSharpBackend {
        ...
        match node {
            ...
            CodegenNode::Method { name, params, return_type, body, is_async, is_static, visibility, decorators: _ } => {
                ...
                let mut result = format!("{}{} {}{}{} {}({}) {{\n",
                    ctx.get_indent(), vis, static_kw, async_kw, return_str, name, params_str);
            ...
            CodegenNode::Constructor { params, body, super_call } => {
                ...
                let mut result = format!("{}public {}({}){} {{\n", ctx.get_indent(), class_name, params_str, base_call);
                ctx.push_indent();
                for stmt in body {
                    result.push_str(&self.emit(stmt, ctx));
                    if self.needs_semicolon(stmt) {
            ...
            CodegenNode::Dict(pairs) => {
                ...
                format!("new Dictionary<string, object> {{ {} }}", pairs_str.join(", "))
            }
    ...
    fn runtime_imports(&self) -> Vec<String> {
        vec!["using System;".to_string(), "using System.Collections.Generic;".to_string()]
    }

    fn class_syntax(&self) -> ClassSyntax { ClassSyntax::csharp() }
    fn target_language(&self) -> TargetLanguage { TargetLanguage::CSharp }
