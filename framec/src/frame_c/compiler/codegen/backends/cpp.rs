    ...
    fn emit(&self, node: &CodegenNode, ctx: &mut EmitContext) -> String {
        match node {
            ...
            CodegenNode::Class { name, fields, methods, base_classes, is_abstract: _, .. } => {
                ...
                let private_fields: Vec<_> = fields.iter().filter(|f| !matches!(f.visibility, Visibility::Public)).collect();
                let private_methods: Vec<_> = methods.iter().filter(|m| {
                ...
                let public_fields: Vec<_> = fields.iter().filter(|f| matches!(f.visibility, Visibility::Public)).collect();
                let public_methods: Vec<_> = methods.iter().filter(|m| {
                    matches!(m, CodegenNode::Constructor { .. }) || matches!(m, CodegenNode::Method { visibility: Visibility::Public, .. })
                }).collect();
            ...
            CodegenNode::Method { name, params, return_type, body, is_async: _, is_static, visibility: _, .. } => {
                let static_kw = if *is_static { "static " } else { "" };
                let return_str = self.map_type(return_type.as_ref().unwrap_or(&"void".to_string()));
                let params_str = self.emit_params(params);

                let mut result = format!("{}{}{} {}({}) {{\n",
                    ctx.get_indent(), static_kw, return_str, name, params_str);
            ...
            CodegenNode::Constructor { params, body, super_call } => {
                ...
                let params_str = self.emit_params(params);
                let init_list = super_call.as_ref().map(|sc| {
                    format!(" : {}", self.emit(sc, ctx))
                }).unwrap_or_default();
            ...
            CodegenNode::Dict(pairs) => {
                ...
                format!("std::unordered_map<std::string, std::any>{{ {} }}", pairs_str.join(", "))
            }
