...
impl LuaBackend {
    ...
    /// Transform Frame brace blocks to Lua if/then/end, while/do/end.
    /// Also strips unreachable code after `return` within blocks
    /// (Lua grammar forbids statements after return in a block).
    fn transform_blocks(text: &str) -> String {
        let mut result = String::new();
        let mut depth = 0i32;
        let mut after_return = false;
        let mut return_depth = 0i32;

        for line in text.lines() {
            let t = line.trim();
            let indent = if t.is_empty() { "" } else { &line[..line.len() - t.len()] };

            // Block openers reset after_return tracking
            if t.starts_with("if ") && t.ends_with('{') {
                after_return = false;
                let cond = t[3..t.len()-1].trim();
                result.push_str(&format!("{}if {} then\n", indent, cond));
                depth += 1; continue;
            }
            if t == "} else {" || t == "}else{" {
            ...
            if t.starts_with("} else {") && t.contains(" if ") {
                after_return = false;
                if let Some(if_pos) = t.find(" if ") {
                    let rest = &t[if_pos+4..];
                    if rest.ends_with('{') {
                        let cond = rest[..rest.len()-1].trim();
                        result.push_str(&format!("{}elseif {} then\n", indent, cond));
                        continue;
                    }
                }
            ...
            if t.starts_with("while ") && t.ends_with('{') {
                after_return = false;
                let cond = t[6..t.len()-1].trim();
                result.push_str(&format!("{}while {} do\n", indent, cond));
                depth += 1; continue;
            }
            if (t == "}" || t == "end") && depth > 0 {
                // Closing a block — if we were in after_return at this depth, reset
                if after_return && depth <= return_depth {
                    after_return = false;
                }
                depth -= 1;
                result.push_str(&format!("{}end\n", indent));
            ...
            // Also handle end/elseif/else as block structure resets
            if t == "end" || t.starts_with("elseif ") || t == "else" {
                after_return = false;
            }

            // Reset after_return at block boundaries (elseif/else/end start new contexts)
            if after_return && (t.starts_with("elseif ") || t == "else" || t == "end") {
                after_return = false;
            }

            // Skip unreachable code after return (Lua grammar restriction)
            if after_return {
