...
pub(crate) fn generate_persistence_methods(
    ...
    match syntax.language {
        TargetLanguage::Python3 => {
            ...
            // Python persist uses field-by-field JSON (the same wire
            // shape as TS/JS/Ruby/Lua/PHP/Dart) — not whole-object
            // pickle. The blob is `bytes` (UTF-8 JSON). This keeps the
            // wire format uniform across backends and makes
            // `@@[no_persist]` a per-field skip.
