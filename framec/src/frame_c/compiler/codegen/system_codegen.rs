...
for comp in reversed(chain):
    ...
    // __kernel method - the main event processing loop. Per-handler
    // architecture with HSM cascades: the kernel synthesizes <$ and $>
    // events around the compartment switch, and the cascade helpers walk
...
fn generate_ruby_machinery(event_class: &str) -> Vec<CodegenNode> {
    ...
    // __router — per-handler architecture: pass self's active compartment
    // as the second arg so dispatcher + handler methods see the owning
    // state's compartment as a named local (see docs/frame_runtime.md
    // § "Dispatch Model").
...
fn generate_kotlin_machinery(
    ...
    // __router — Kotlin: per-handler architecture passes __compartment
