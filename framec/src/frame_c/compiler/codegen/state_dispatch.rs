...
/// Transform Frame brace blocks to Lua syntax: if/then/elseif/else/end, while/do/end
fn lua_transform_blocks(text: &str) -> String {
    let mut result = String::new();
    let mut block_depth: Vec<&str> = Vec::new(); // "if" or "while"
    ...
    for line in text.lines() {
        ...
        // `} else { if condition {` → `elseif condition then`  (nested elif)
        if (trimmed.starts_with("} else {") || trimmed.starts_with("}else{")) && trimmed.contains("if ") && trimmed.ends_with('{') {
            // Find the inner if
            let after_else = if trimmed.starts_with("} else { if ") {
                &trimmed[12..trimmed.len()-1]
            ...
            // Pop the if we're closing, the elseif continues the chain
            if !block_depth.is_empty() { block_depth.pop(); }
            result.push_str(&format!("{}elseif {} then\n", indent, after_else.trim()));
            block_depth.push("if");
            continue;
        }
        ...
        if trimmed.starts_with("while ") && trimmed.ends_with('{') {
            let condition = trimmed[6..trimmed.len()-1].trim();
            result.push_str(&format!("{}while {} do\n", indent, condition));
            block_depth.push("while");
            continue;
        }
