...
#### `@@:(expr) return;` on one line — the `ContextReturnExpr` segment consumes the trailing `return`

When `@@:(expr)` and `return;` are on the same source line, the native-region scanner extends the `ContextReturnExpr` segment to consume the trailing whitespace + `return` + `;`, and the expansion emits the `_return = …` assignment and the language-appropriate `return` statement on **separate lines**, both at the segment's indent. A new backend's `ContextReturnExpr` arm in `frame_expansion.rs` must check the `has_native_return` flag and append a `return`/`return;` line when it's set (see the existing arms). This was broken pre-rollout (the two were joined with no separator → a syntax error on Swift/Kotlin/Lua/GDScript); fixed in commit `135d5fa` — see § "TODOs surfaced by the rollout" #1. The corpus exercises the one-line form on several backends via the `system_params/sysparam_*` fixtures.
