...
Frame handles this natively, and the framepiler itself is the proof. The compiler uses 35 Frame-generated state machines (`.frs` → `.gen.rs`) for its scanning infrastructure: 14 SyntaxSkipper FSMs for per-language comment and string skipping, 15 BodyCloser FSMs for brace matching, one scope scanner for Erlang `fun...end` closures, three sub-machines for expression scanning, context parsing, and state-variable parsing, and two for the output-block lexer/parser pair. These are the components that handle the "states within states" of recognising lexical structure across 17 target languages. (The 14-vs-15 asymmetry between skippers and body-closers is itself a small piece of evidence for the composition thesis: Go has identical comment and double-quoted-string syntax to Java, so the Go skipper reuses Java's FSM via `include!()` and adds only a small hand-written Rust path for backtick raw strings. One state machine, two target languages — composition over duplication.) The framepiler's stage-2 grammar parser is hand-written Rust recursive descent. Its stage-0 and stage-1 scanning work is Frame. This division is itself evidence for the composition thesis: the places where state machines genuinely help — bounded lexical sub-problems — get expressed in Frame; the place where they don't — the grammar-structural recursive descent — stays in Rust. The division reflects an empirical judgment by the people who know the tool best.
