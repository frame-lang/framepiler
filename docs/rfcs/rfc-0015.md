...
Frame originally ran a [system](../glossary.md#system)'s initialization code
*inside the host language's constructor* — a coupling that made restoring a
persisted system fragile (a persisted child system with required initialization
parameters had nothing to pass when restore tried to re-construct it) and on
some targets impossible (GDScript's `_init` can't be skipped). This RFC moves
[construction](../glossary.md#construction) out of the host constructor: framec
generates an explicit [*factory*](../glossary.md#factory) that allocates a system
instance and runs its [start state](../glossary.md#start-state)'s
initialization; the host-language constructor only allocates and never runs user
code; and the Frame-source expression `@@!Foo()` allocates a system instance
with **no** initialization run — exactly what [restore](../glossary.md#restore)
needs.

So a system instance can come from one of two operations:

- **[Construction](../glossary.md#construction)** — allocate *and* run
  initialization (the [start state](../glossary.md#start-state)'s `$Start(...)`
  body and its [`$>` enter handler](../glossary.md#-enter-handler)). The
  user-facing entry point is the [factory](../glossary.md#factory).
- **[No-initialization allocation](../glossary.md#no-initialization)** — allocate
  and run *nothing*. The user-facing entry point is `@@!Foo()`.
...
A Frame system is a state machine; you use one by creating a running **instance**
of it — and, if the system is persisted, by re-creating an instance from a saved
blob. The original design put that second case at risk. When a system whose
[start state](../glossary.md#start-state) takes parameters — a database handle,
a seed, a configuration object — was held as a domain field of a larger
persisted system, restoring the larger system tried to *re-construct* that inner
system and crashed: it had no parameters to pass to a constructor that demanded
them.
