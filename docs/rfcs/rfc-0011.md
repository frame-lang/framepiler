...
> `SystemAst.bases: Vec<String>` from the `:` clause on `@@system`.
> Each backend's `LanguageSyntax` in `codegen/backend.rs` carries an
> `extends_keyword: Option<String>` driving per-language emission
> (`extends`, `:`, `<`, or `None` for languages without inheritance).
> Normative documentation lives in `docs/frame_language.md` under
> "Base Classes".
