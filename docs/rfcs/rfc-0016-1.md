...
- **Status:** Accepted — the attribute is parsed and validated; codegen (skip the
  field in `save`, leave it at its `domain:` default on `load`) is **not yet
  implemented** on any backend. See `CHANGELOG.md` when it lands.
