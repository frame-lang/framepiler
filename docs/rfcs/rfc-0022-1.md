...
> **Superseded:** This RFC reduced `@@import` to a
> dependency-declaration-only directive on Java/C#/Go.
> [RFC-0024](rfc-0024.md) removes `@@import` from the language
> entirely; cross-file dependencies are now expressed using the
> target host language's native import syntax as
> [Oceans Model](../glossary.md#oceans-model) pass-through on
> every backend. The Java/C#/Go behavior this RFC documented is
> generalized to all 17 targets.
