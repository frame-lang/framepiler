...
Call-site lowering reorganized:

- `@@Counter(7)` → `Counter::__create(7)` (or equivalent per backend)
- `@@!Counter()` → bare constructor invocation
...
D7's `@@!Foo()` semantics are unchanged — only the per-backend mechanism is simpler. The RFC-0015 D7 section in `docs/rfcs/rfc-0015.md` is updated to reflect the new lowering.
